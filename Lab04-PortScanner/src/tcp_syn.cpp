...
bool tcpSynScan(const std::string& host_ip, const uint16_t port) {
    // Create socket
    ...
    if (sendto(sockfd, tcp_header, sizeof(struct tcphdr), 0, (struct sockaddr*)&dest, sizeof(dest)) <= 0) {
        std::cerr << "[Error] Failed to send TCP SYN packet: " << strerror(errno) << std::endl;
        close(sockfd);
        delete[] buffer;
        return false;
    }
    ...
    while (true) {
        ...
        if (ret > 0) {
            ...
            // Check if the received packet is a TCP SYN-ACK packet
            if (recv_tcp_header->th_flags == (TH_SYN | TH_ACK)) { // SYN | ACK
                std::cout << "TCP SYN Scan " << host_ip << ":" << port << " Port open" << std::endl;
                close(sockfd);
                delete[] buffer;
                return true;
            } else if (recv_tcp_header->th_flags == TH_RST) { // RST
                std::cout << "TCP SYN Scan " << host_ip << ":" << port << " Port closed" << std::endl;
                close(sockfd);
                delete[] buffer;
                return false;
            } else {
                std::cout << "TCP SYN Scan " << host_ip << ":" << port << " Unknown response" << std::endl;
            }
        }
        ...
        if (duration > TIMEOUT_MS) {
            std::cout << "TCP SYN Scan " << host_ip << ":" << port << " Timeout (May be filtered)" << std::endl;
            close(sockfd);
            delete[] buffer;
            return false;
        }

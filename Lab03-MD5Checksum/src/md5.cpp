...
/*
 * Encodes input (uint32_t) into output (uint8_t). Assumes length(in bytes) is a multiple of 4.
 */
...
/*
 * Decodes input (uint8_t) into output (uint32_t). Assumes length(in bytes) is a multiple of 4.
 */
...
/*
 * Copies data from input to output. Assumes that the input and output buffers do not overlap.
 */
void MD5::MD5_memcpy(uint8_t *output, const uint8_t *input, size_t length) {
 ...
 * Set output to value.
 */
void MD5::MD5_memset(uint8_t *output, uint8_t value, size_t length) {
    for (size_t i = 0; i < length; i++) {
...
void MD5::GetHash(uint8_t* &hash) {
    MD5_memcpy(hash, digest, 16);
}

/*
 * Get the hash as a string.
 */
std::string MD5::GetHashStr() {
    std::string str;
    str.reserve(32);

    for (size_t i = 0; i < 16; i++) {
        str.push_back("0123456789abcdef"[digest[i] >> 4]);
        str.push_back("0123456789abcdef"[digest[i] & 0x0f]);
    }
